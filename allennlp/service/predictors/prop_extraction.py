...
from allennlp.service.predictors.spacy_wrapper import spacy_wrapper
    ...
    def get_extractions(self, sent):
        ...
        for verb in [i for i in range(self.parser.get_len()) if self.parser.is_verb(i)]:
            ...
            for child in self.parser.get_children(verb):
                ...
                if extraction.pred_exists and self.parser.is_part_of_non_consecutive_span(verb, curr_arg):
...
class Extraction:
    ...
    def __str__(self):
        ...
        ret = '\t'.join([str(self.pred_index), self.template, self.lemmatized_template] +
                        ['A{}\t{}'.format(key, val)
                         for key, val in sorted(iter(self.roles_dict.items()), key = lambda k__: k__[0])])
        return ret
